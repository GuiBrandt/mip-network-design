...
public:
  ...
  // Get information from the header table (the first table with only one line)
  bool Get(string colname,string &info);
  bool Get(string colname,int &info);
  bool Get(string colname,double &info);
  // Get information from the nodes or edges
  bool Get(string colname,NodeValueMap &col);
  bool Get(string colname,NodeIntMap &col);
  bool Get(string colname,NodeStringMap &col);
  bool Get(string colname,EdgeValueMap &col);
  bool Get(string colname,EdgeIntMap &col);
  bool Get(string colname,EdgeStringMap &col);
  void print();
...
inline bool GraphTable::Get(string colname,string &info){
  return(this->Header->first(colname,info));}
inline bool GraphTable::Get(string colname,int &info){
  return(this->Header->first(colname,info));}
inline bool GraphTable::Get(string colname,double &info){
  return(this->Header->first(colname,info));}


inline bool GraphTable::Get(string colname,NodeValueMap &column){
  ...
  return(true);}
inline bool GraphTable::Get(string colname,NodeIntMap &column){
  int col = this->NodeTable->columnindex(colname);
  if (col==-1) {cout << "Error: Column name \""+colname+"\" missing.\n";exit(0);}
  for (int i=0;i<this->NodeTable->nrows;i++)
    this->NodeTable->entry(i,col,column[this->line2node[i]]);
  return(true);}
inline bool GraphTable::Get(string colname,NodeStringMap &column){
  int col = this->NodeTable->columnindex(colname);
  if (col==-1) {cout << "Error: Column name \""+colname+"\" missing.\n";exit(0);}
  for (int i=0;i<this->NodeTable->nrows;i++)
    this->NodeTable->entry(i,col,column[this->line2node[i]]);
  return(true);}
inline bool GraphTable::Get(string colname,EdgeValueMap &column){
  ...
  return(true);}
inline bool GraphTable::Get(string colname,EdgeIntMap &column){
  int col = this->EdgeTable->columnindex(colname);
  if (col==-1) {cout << "Error: Column name \""+colname+"\" missing.\n";exit(0);}
  for (int i=0;i<this->EdgeTable->nrows;i++)
    this->EdgeTable->entry(i,col,column[this->line2edge[i]]);
  return(true);}
inline bool GraphTable::Get(string colname,EdgeStringMap &column){
  int col = this->EdgeTable->columnindex(colname);
  if (col==-1) {cout << "Error: Column name \""+colname+"\" missing.\n";exit(0);}
  for (int i=0;i<this->EdgeTable->nrows;i++)
    this->EdgeTable->entry(i,col,column[this->line2edge[i]]);
...
public:
  ...
  bool Get(string colname,int &info);
  bool Get(string colname,double &info);
  // Get information from the nodes or arcs
  bool Get(string colname,DNodeValueMap &col);
  bool Get(string colname,DNodeIntMap &col);
  bool Get(string colname,DNodeStringMap &col);
  bool Get(string colname,ArcValueMap &col);
  bool Get(string colname,ArcIntMap &col);
  bool Get(string colname,ArcStringMap &col);
  void print();
  ...
  g(graph)
{
  ...
  int nnodes; Header->first("nnodes", nnodes);
  int narcs;  Header->first("narcs",narcs);
...
inline bool DigraphTable::Get(string colname,string &info){
  return(this->Header->first(colname,info));}
inline bool DigraphTable::Get(string colname,int &info){
  return(this->Header->first(colname,info));}
inline bool DigraphTable::Get(string colname,double &info){
  return(this->Header->first(colname,info));}

inline bool DigraphTable::Get(string colname,DNodeValueMap &column){
  int col = this->NodeTable->columnindex(colname);
  if (col==-1) {cout << "Error: Column name \""+colname+"\" missing.\n";exit(0);}
  for (int i=0;i<this->NodeTable->nrows;i++)
    this->NodeTable->entry(i,col,column[this->line2node[i]]);
  return(true);}
inline bool DigraphTable::Get(string colname,DNodeIntMap &column){
  int col = this->NodeTable->columnindex(colname);
  if (col==-1) {cout << "Error: Column name \""+colname+"\" missing.\n";exit(0);}
  for (int i=0;i<this->NodeTable->nrows;i++)
    this->NodeTable->entry(i,col,column[this->line2node[i]]);
  return(true);}
inline bool DigraphTable::Get(string colname,DNodeStringMap &column){
  int col = this->NodeTable->columnindex(colname);
  if (col==-1) {cout << "Error: Column name \""+colname+"\" missing.\n";exit(0);}
  for (int i=0;i<this->NodeTable->nrows;i++)
    this->NodeTable->entry(i,col,column[this->line2node[i]]);
  return(true);}
inline bool DigraphTable::Get(string colname,ArcValueMap &column){
  ...
  return(true);}
inline bool DigraphTable::Get(string colname,ArcIntMap &column){
  int col = this->ArcTable->columnindex(colname);
  if (col==-1) {cout << "Error: Column name \""+colname+"\" missing.\n";exit(0);}
  for (int i=0;i<this->ArcTable->nrows;i++)
    this->ArcTable->entry(i,col,column[this->line2arc[i]]);
  return(true);}
inline bool DigraphTable::Get(string colname,ArcStringMap &column){
  int col = this->ArcTable->columnindex(colname);
  if (col==-1) {cout << "Error: Column name \""+colname+"\" missing.\n";exit(0);}
  for (int i=0;i<this->ArcTable->nrows;i++)
    this->ArcTable->entry(i,col,column[this->line2arc[i]]);
