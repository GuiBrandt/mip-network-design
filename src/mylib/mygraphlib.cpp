			     ...
			     NodePosMap   &posy)
{
  ...
  out << "}\n";
  out.close();
  sprintf(cmd,"%s -q -Goverlap=false %s -o %s",mygraphlibparameters.graphvizdrawingprogram.c_str(),tempnamedot,tempnameposdot); 
  fflush(stdout);
		      ...
		      EdgeValueMap& weight)
{
  ...
  bool ok = GT.Get(mygraphlibparameters.inputgraphtable_nodename,vname);
  ok = ok && GT.Get("weight",weight);
  ...
  g(GT.g),node_name(GT.g),px(GT.g),py(GT.g),node_attrib(GT.g),edge_attrib(GT.g)
{
  factor = 1.0;
  graph_attrib = "";
  GT.Get(mygraphlibparameters.inputgraphtable_nodename,node_name);
  ...
  GT.Get("nodename",this->node_name);
...
bool GraphAttributes::View() // text displayed below the figure
{
  ...
  fprintf(fp,"}\n");
  fclose(fp);
  sprintf(cmd,"%s -q -Tpdf %s -o %s",mygraphlibparameters.graphvizdrawingprogram.c_str(),tempname,outputname);
system(cmd);
