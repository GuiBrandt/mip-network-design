...
// This cutting plane routine finds/inserts violated cuts between the root and the other terminals.
...
protected:
  void callback()
  {
    ...
    // AULA
    // Experimente comparar a execucao comentando ou deixando as proximas duas linhas.
	...
	capacity[a]=(this->*solution_value)(x[a]);
...
bool ReadSteinerDigraph(string filename,Digraph &dg,DNodeStringMap& vname,
  ...
  bool ok = GT.Get("nodename",vn);
  ok = ok && GT.Get("weight",w);
  ok = ok && GT.Get("terminal",is_terminal);
