...
{ // Although capacity and other vectors are only defined for part of the nodes, using this way
  ...
  int nf; MainParameters.first("nfacilities",nf);
  int nc; MainParameters.first("nclients",nc);
  int ne; MainParameters.first("nconnectioncosts",ne);
...
int main(int argc, char *argv[])
{
  ...
  // Exercicio em Aula
  //=======================================================
  // Mude a prioridade de branching das variaveis.
  // Faca experimentos e procure instancias que fazem a diferenca.
  // Variaveis com valores de BranchPriority maiores tem maior preferencia
  // para entrar no branching.
  // 
  for (int i=0;i<nf;i++) y[Facility[i]].set(GRB_IntAttr_BranchPriority,1);
  for(ArcIt e(g); e != INVALID; ++e) x[e].set(GRB_IntAttr_BranchPriority,1000);
